...
WGBASTCode<-nimbleCode({ 
  ...
  for(s in 1:stocks){
    ...
    simm[1,1,s]<-exp(-(11*MpsW[1,AU[s]]/Tstep))*exp(-(MpsW[1,AU[s]]*sealMort[1,1,1]/Tstep))     #note AU 1 sealMort for all change later
    ...
    for(i in 2:(m)){
      ...
      simm[i,1,s]<-exp(-(11*MpsW[i,AU[s]]/Tstep))*exp(-(MpsW[i,AU[s]]*sealMort[i,1,1]/Tstep))   #note AU 1 sealMort for all change later
  ...
  for (i in 1:m){                    ### Tagging data likelihoods ### 
    ...
    for (j in 1:6){
      ...
      #ncrR[i,j]<- HrR[i,j]*NrR_all[i,j]  #reared salmon total catch
...
#      for(s in 1:2){
#         ncrW[i,j,s]<-HrW[i,j,s]*NladderW[i,j,s]+NladderRsp[i,j,s]*HrW[i,j,s]    #rivHR needed?
#      }
#      for(s in 3:stocks){
#         ncrW[i,j,s]<-HrW[i,j,s]*NladderW[i,j,s]
#      }
...
#  MR ~dlnorm(-2.3, 4.3)
   ...
   for(st in 1:AUS){
    ...
    for(t in 1:(m-1)){
        mu_surv[t,1:AUS]<-logit_survMps[t,1:AUS]
        logit_survMps[(t+1),1:AUS]~dmnorm(mu_surv[t,1:AUS],tau_Mps[1:AUS,1:AUS]) 
        for(st in 1:AUS){
    ...
    ##############################################
        ...
        R[1,2]     <- r12               
        R[2,2]     <- sqrt(1 - r12^2)   
        R[2:AUS,1]   <- 0    #with > 2 stocks (plus lines below)
          
          for (i in 2:(AUS-1)) {         #NB stocks needs to be >2, otherwise just use lines above!!
        ...
        Rnew[1:AUS,1:AUS]<-t(R[1:AUS,1:AUS]) %*% R[1:AUS,1:AUS] 
        ##
        for(st in 1:AUS){
            sigma_stock[st]  ~ T(dlnorm(-1.61,4),0.001,0.50)    #change upper bound later?
            for(j in 1:AUS){
                sigma_st[st,j] <- Rnew[st,j] * sigma_stock[st] * sigma_stock[j]
            }
        }
 
    tau_Mps[1:AUS, 1:AUS]<- inverse(sigma_st[1:AUS, 1:AUS])
  ...
  for(i in 1:(m)){
    for(s in 1:stocks){
            survMps[i,1,s]<-exp(-MpsW[i,AU[s]]/Tstep) 		          #survmortW
            survMps[i,2,s] <- exp(-qlW[i,1]*El[i,1]) * exp(-2*MpsW[i,AU[s]]/Tstep) 	       #survlW  
            survMps[i,3,s] <- exp(-F_sea[i,1,1,AU[s]])* exp(-(6*MpsW[i,AU[s]]/Tstep))   	      #was survc
            survMps[i,4,s] <- exp(-qdW[i,1]*Edo[i,1]) * exp(-MpsW[i,AU[s]]/Tstep)         #survdoW
            survMps[i,5,s] <- (1-HrW[i,1])*exp(-MpsW[i,AU[s]]*sealMort[i,1,1]/Tstep)  #was surv.riv
            survMps[i,6,s]<-(1-HtW[i,1]*(p.rel[(i+1-1)]*p.mort+(1-p.rel[(i+1-1)])))* exp(-MpsW[i,AU[s]]/Tstep) 
        }
